describe('Dataset', () => {
    ...
    it('Can see resource pages', () => {
        cy.visit(
            '/dataset/2015-gsa-common-baseline-implementation-plan-and-cio-assignment-plan'
        );
    ...
    it('Can get harvest information via API', () => {
        cy.request(
            '/api/action/package_show?id=ek500-water-column-sonar-data-collected-during-al0001'
        ).should((response) => {
    ...
    it('Can click on items on the sidebar (e.g. tags filter)', () => {
        ...
        cy.get('div[class="filter-list"] span[class="filtered pill"]').contains(
            'test-harvest-org'
        );
    });
    ...
    it("Can click on items on the dataset's sidebar (e.g. )", () => {
        cy.visit(
            '/dataset/ek500-water-column-sonar-data-collected-during-al0001'
        );
        cy.get('a[class="heading"]')
            .contains('NCEI Contact Information')
            .click();
