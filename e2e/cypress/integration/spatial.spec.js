describe('Spatial', () => {
    ...
    it('Can search geographies by name', () => {
        cy.request('/api/3/action/location_search?q=california').should(
            (response) => {
    ...
    it('The map view works and can draw box and search', () => {
        ...
        cy.get('#dataset-map-edit-buttons')
            .find('[class="btn apply btn-primary"]')
            .click();
    ...
    it('Can search in the location dropdown', () => {
        ...
        cy.visit('/dataset');
        cy.get('#dataset-map-edit')
            .find('span')
            .contains('Enter location...')
            .click();
        cy.get('#select2-drop').find('input').type('wash');
        cy.get('#select2-results-1')
            .find('div')
            .contains('Washington, DC')
            .click();
        cy.url().should(
            'include',
            'ext_location=Washington%2C+DC+%2820001%29&ext_bbox=-77.0275'
        );
    ...
    it('Can put a package with weird tags in an group', () => {
        ...
        cy.request({
            ...
            expect(response.body).to.have.property('success', true);
            expect(response.body.result.groups[0]).to.have.property(
                'name',
                'climate'
            );
    ...
    it('Can parse a harvest source with spacial tags in a list', () => {
        cy.request(
            ...
            expect(spatail_info).to.have.property(
                'old-spatial',
                '[[-14.601813, -179.231086], [71.441059, 179.859681]]'
            );
            expect(spatail_info).to.have.property(
                'spatial',
